"""Download and query the plugin-local Danbooru vocabulary snapshot."""
...
def read_cache_info(path: Path) -> DanbooruCacheInfo | None:
    """Read and validate local cache metadata.

    Args:
        path: SQLite cache path.
...
def lookup_local_tags(
    ...
    """
    if read_cache_info(path) is None:
...
async def update_danbooru_cache(
    ...
    headers = {"User-Agent": "AstrBot-NovelAI/3.2 (local tag cache updater)"}
    try:
    ...
    temporary_path = path.with_suffix(".sqlite3.tmp")
